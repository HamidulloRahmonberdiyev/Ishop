...
class OrderController extends AppController {
    ...
    public function indexAction() {
        ...
        `order_product`.`order_id` GROUP BY `order`.`id` ORDER BY `order`.`status`, `order`.`id`
        LIMIT $start, $perpage");
        
        $this->setMeta('Panel control');
    ...
    public function viewAction() {
        ...
        ROUND(SUM(`order_product`.`price`), 2)AS `sum` FROM `order` JOIN `user`
         ON `order`.`user_id` = `user`.`id` JOIN `order_product` ON `order`.`id` =
        `order_product`.`order_id` WHERE `order`.`id` = ? GROUP BY 
        `order`.`id` ORDER BY `order`.`status`, `order`.`id` LIMIT 1", [$order_id]);
            if (!$order) {
    ...
    public function deleteAction() {
        ...
        $order = \R::load('order', $order_id);  
        \R::trash($order);
        $_SESSION['success'] = 'Deleted order';
